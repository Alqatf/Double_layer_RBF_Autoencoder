function [cost,grad] = sparseAutoEncoderLayerCost(theta, inputSize, hiddenSize, visibleSize,...
                                             lambda, sparsityParam, beta, initialData,data)

% visibleSize: the number of input units (probably 64) 
% hiddenSize: the number of hidden units (probably 25) 
% lambda: weight decay parameter
% sparsityParam: The desired average activation for the hidden units (denoted in the lectur
...
W2 = reshape(theta(hiddenSize*inputSize+1:hiddenSize*(inputSize+visibleSize)), visibleSize, hiddenSize);
b1 = theta(hiddenSize*(inputSize+visibleSize)+1:hiddenSize*(inputSize+visibleSize)+hiddenSize);
b2 = theta(hiddenSize*(inputSize+visibleSize+1)+1:end);
...
cost_main = (0.5/sample_num)*sum(sum((initialData - output_2).^2));%error term in the cost
...
errorterm_2 = -(initialData - output_2).*sigmoidInv(input_2);
